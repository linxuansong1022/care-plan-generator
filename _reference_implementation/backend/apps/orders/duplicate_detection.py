"""
Duplicate detection for orders, patients, and providers.
"""
...
class ProviderDuplicateDetector:
    ...
    def check(npi: str, name: str) -> DuplicateCheckResult:
        ...
        similar = (
            Provider.objects.annotate(lower_name=Lower("name"))
            .filter(lower_name__icontains=name.lower().split()[0])
            .exclude(npi=npi)[:5]
        )
        ...
        if similar.exists():
            warnings.append(
                Warning(
                    ...
                    action_required=False,
                    data={"similar_providers": [{"npi": p.npi, "name": p.name} for p in similar]},
                )
            )
            return DuplicateCheckResult(
...
class PatientDuplicateDetector:
    ...
    @staticmethod
    def check(
        mrn: str, first_name: str, last_name: str, date_of_birth=None
    ) -> DuplicateCheckResult:
        """
        Check for patient duplicates.

        Rules:
        - Same MRN + same fn/ln/dob -> use existing (OK)
        - Same MRN + different fn/ln/dob -> WARN
        - Same fn + ln + dob, different MRN -> WARN (potential duplicate)
        """
        warnings = []
        ...
        try:
            ...
            names_match = (
                existing.first_name.lower().strip() == first_name.lower().strip()
                and existing.last_name.lower().strip() == last_name.lower().strip()
            )
            dob_match = (
                date_of_birth is None
                or existing.date_of_birth is None
                or existing.date_of_birth == date_of_birth
            )
            ...
            if names_match and dob_match:
                ...
                # Both name and DOB mismatch - WARNING (can confirm)
                warnings.append(
                    Warning(
                ...
                return DuplicateCheckResult(
                    is_duplicate=True,
                    is_potential_duplicate=True,  # This triggers WARNING flow
                    existing_record=existing,
                    warnings=warnings,
                )
            elif not names_match:
                # Name mismatch - WARNING (can confirm)
                warnings.append(
                    Warning(
                        ...
                        action_required=True,
                        data={"existing_name": f"{existing.first_name} {existing.last_name}"},
                    )
                )
                return DuplicateCheckResult(
                    is_duplicate=True,
                ...
                # DOB mismatch only - WARNING (can confirm)
                warnings.append(
                    Warning(
            ...
            # names_match and dob_match - use existing patient
            return DuplicateCheckResult(
                is_duplicate=True,
                existing_record=existing,
                warnings=warnings,
...
class OrderDuplicateDetector:
    ...
    def check(
        ...
        Rules:
        - Same patient + same medication + same day -> ERROR (block)
        - Same patient + same medication + different day -> WARNING (can confirm)
        """
...
class FullDuplicateCheckResult:
    ...
    def has_blocking_issues(self) -> bool:
        return (
            self.provider_result.should_block
            or self.patient_result.should_block
            or (self.order_result and self.order_result.should_block)
        )

    @property
    def requires_confirmation(self) -> bool:
        def needs_confirm(result: DuplicateCheckResult) -> bool:
            return result.is_potential_duplicate and any(
                w.action_required for w in result.warnings
            )

        return (
            needs_confirm(self.provider_result)
            or needs_confirm(self.patient_result)
            or (self.order_result and needs_confirm(self.order_result))
        )
